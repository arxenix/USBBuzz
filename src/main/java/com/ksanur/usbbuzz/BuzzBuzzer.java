...
public class BuzzBuzzer {
    ...
    /**
     * Recursively searches for the buzzer device on the specified USB
     * hub and returns it. If there are multiple buzzers attached then
     * this simple demo only returns the first one.
     *
     * @param hub
     *            The USB hub to search on.
     * @return The buzzer USB device or null if not found.
     */
    public static UsbDevice findBuzzer(UsbHub hub)
    {
        ...
        for (UsbDevice device: (List<UsbDevice>) hub.getAttachedUsbDevices())
        {
            if (device.isUsbHub())
            {
                launcher = findBuzzer((UsbHub) device);
